...
import { CreateJobOpeningDto } from '../presentation/dto/create-job-opening.dto';
import { UpdateJobOpeningDto } from '../presentation/dto/update-job-opening.dto';
import {
...
export class JobOpeningService {
  ...
  async create(dto: CreateJobOpeningDto, user: AuthenticatedUser) {
  ...
  async findAll(user: AuthenticatedUser, query: PaginationQueryDto = {}) {
    ...
    const [jobOpenings, totalItems] = await this.prisma.$transaction([
    ...
    return buildPaginatedResult(jobOpenings, totalItems, page, pageSize);
  }
  ...
  async findAllPublicOpen() {
    ...
    return jobOpenings.map(({ company, ...rest }) => ({
      ...rest,
  ...
  async update(id: string, dto: UpdateJobOpeningDto, user: AuthenticatedUser) {
    const jobOpening = await this.prisma.jobOpening.findFirst({
      where: { id, companyId: user.companyId },
    });
  ...
  async getOpenJobOpeningForApply(code: string) {
    ...
    if (
      ...
      jobOpening.company.status !== 'ACTIVE'
    ) {
  ...
  private async createWithUniquePublicCode(
    ...
    throw new ConflictException(
      'Não foi possível gerar um código público único para a vaga',
    );
  }
}
